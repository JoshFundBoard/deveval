...
function postChoices({ name, choices }) {
  /*
  ...
  */
  ...
  data.records.forEach(r => {
    if (!r.fields.name.trim()) throw new Error('A name is required.');
...
function* workPostChoices({ payload }) {
  try {
    const result = yield call(postChoices, payload);
    const { data } = result;
    ...
    yield put({ type: types.POST_CHOICES_SUCCEEDED, data });
  } catch (error) {
    yield put({ type: types.POST_CHOICES_FAILED, error });
  }
}

export function* watchPostChoices() {
  yield takeEvery(types.POST_CHOICES_REQUESTED, workPostChoices);
}
